			...
			<tbody>
				{#each types as item}
					<tr class="bg-gradient-to-b from-transparent to-slate-100">
						...
						<td class="p-2">
							<span
								><a href={`/treatments/chemicals/types/${item.id}`} class="text-blue-600 underline"
									>{item.name}</a
								></span
							>
						</td>
						...
						<td class="p-2"
							><a
								...
								href={`/treatments/chemicals/types/${item.id}/edit`}>edit</a
							></td
						>
					</tr>
				{/each}
